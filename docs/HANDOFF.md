...
**State**
- `main` = `d3047e9` (PR #66, merged with Paige's explicit "merge to main" go-ahead after #59 item 1 landed properly), deployed and confirmed live. `live-producer-preview` had one further commit (`477a26d`, #59 items 4/5) verified via clean CI but **not yet promoted** — merge that whenever Paige is ready, same pattern as always.
- **#59 is now fully addressed except the one thing that genuinely needs a real user:** items 1 (0 favorites by default), 2 (single first-run CTA), 4 (de-emphasized nav pre-onboarding), 5 (skip the Look picker before Favorites), and 6 (copy) are all done and merged (item 4/5 as of `477a26d`, not yet promoted — see above). Only the free-form new-user comprehension testing itself is left, and that needs someone other than Paige.
- **#60: only the favorites-pool addition shipped.** The followUpPool expansion stays open on purpose — Paige's call was not to force it while local Chrome is flaky; do it properly (expand the pool *and* fix `bookmark-flow.js`'s hardcoded round-size assumptions in the same pass) once Chrome is stable enough to iterate against the real test.
- **#28 ran: 4/10 calls succeeded, 6 fell back, cost $0.058.** The important finding isn't "weak output" — the 4 that succeeded were well-grounded, specific, non-repetitive, correctly cautious. **All 6 failures are one root cause**: the model prefixes its JSON with prose ("Looking at...") and `parseModelJson` in `api/recommendations.mjs` can't handle a non-JSON preamble, so `JSON.parse` throws and it falls back. This means roughly 60% of live requests currently pay for a call whose output gets discarded. Full writeup with two candidate fixes (tighten the prompt vs. make the parser extract the first `{...}` block) is on the #28 issue comment — **not fixed yet**, deliberately, per Paige's "show the baseline first" instruction. This is probably the next genuinely important product work, ahead of more UI polish.
- **#15 stays parked** until Paige has time to watch/read/play the candidates.
- **Live AI is ON in production and stays on** (Paige: "I'm ok with the gate, no one is using this but me" — settled, don't re-ask). Live job: choose and explain the next recommendation set on "Keep discovering". Hypothesis inference is still deterministic. Model `claude-sonnet-5`, ~$0.014/call — but see the #28 finding above: a real chunk of calls are currently wasted on unparseable output.
...
1. **The #28 JSON-parsing fix is probably the highest-value next thing.** ~60% of live requests currently fall back because the model's response isn't strict JSON (prefixed with prose). Two candidate fixes are written up on the #28 comment; neither has been done yet (deliberately — "show the baseline first"). Worth Paige's explicit go-ahead on which fix (or both) before touching `api/recommendations.mjs`'s prompt or `parseModelJson`.
2. Merge `live-producer-preview`'s one remaining commit (`477a26d`, #59 items 4/5) to `main` when Paige is ready.
3. Get Paige's read on this session's issue comments and close whichever ones she's happy with — a merge doesn't do that automatically. Full list touched this session: #33, #30, #37, #36, #39, #19, #18, #29, #8, #48, #52, #53, #54, #25, #57, #58, #59, #60, #28.
4. Run `npm run test:full` locally when Chrome isn't fighting other sessions on this machine — the real next step for #58 (320-390px widths, all four looks) and for #60's followUpPool expansion (needs a `bookmark-flow.js` rewrite to stop hardcoding exact round sizes, done *alongside* the expansion, not before it).
5. **#15** (Experiment 004) — stays parked; waiting on Paige to actually watch/read/play the 5 locked candidates. Nothing to build.
6. **#48** (hard-boundary exclusions) — deliberately left unbuilt; pick up only if Paige wants it.
7. Parked per earlier decisions, revisit only if Paige changes her mind: #6, #7, #9, #10, #11, #17.
