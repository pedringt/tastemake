...
**State**
- `main` = `357d9e0` (PR #56, merged with Paige's explicit "merge to main" go-ahead), deployed and confirmed live (Vercel status: "Deployment has completed"). **`live-producer-preview` is three commits ahead** (`63704df` #25/#57/#58, `de4a9b0` #59-partial + #60-partial, `f22573e` reverting the broken part of #60 — see below), not yet promoted.
- **New this session, after #25/#57/#58: #59 (first-run onboarding) and #60 (broaden the seed catalog).** #59 got a small, safe slice (single Favorites CTA pre-onboarding via a new `state.onboarded` flag); the rest is flagged as real decision points, not built. #60's favorites-pool addition landed clean; its followUpPool addition was caught by the diagnostic-PR pattern breaking `bookmark-flow.js`'s hardcoded round-composition assumptions and was reverted — see the issue comments and "What's next" below for the honest state of both.
...
1. Run `npm run test:full` locally when Chrome isn't fighting other sessions on this machine — that's the real next step for #58 (320-390px widths, all four looks), and would also let #60's followUpPool expansion and a `bookmark-flow.js` rewrite actually be iterated on instead of guessed at.
...
3. Decide item 1 of #59 (start Favorites empty for real users) — needs a specific call on the QA-fixture-bypass mechanism before it's buildable; see the #59 issue comment for the exact tradeoff.
4. Get `live-producer-preview`'s three remaining unpromoted commits (`63704df`, `de4a9b0`, `f22573e`) merged to `main` when Paige is ready — same pattern as before, needs her explicit go-ahead.
5. **#28** — the free infrastructure already exists (scale-50/100 fixtures, a repetition scorer). The real next step is a paid `--producer endpoint` eval run (~$0.20/10 calls); needs Paige's yes on cost first.
6. **#15** (Experiment 004) — waiting on Paige to actually watch/read/play the 5 locked candidates and report reactions. Nothing to build.
