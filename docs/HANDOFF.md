...
**State**
- `main` = `357d9e0` (PR #56, merged with Paige's explicit "merge to main" go-ahead), deployed and confirmed live (Vercel status: "Deployment has completed"). **`live-producer-preview` is five commits ahead** (`63704df` #25/#57/#58, `de4a9b0` #59-partial + #60-partial, `f22573e` reverting the broken part of #60, `34d36f4` + `797554a` the real #59 item-1 fix — see below), not yet promoted. **Paige explicitly held the merge** until #59/#60 were in better shape, which is why this is still sitting on the branch.
- **#59: real first-run users now start with 0 favorites (item 1, done properly).** Paige's instruction was explicit: seeded favorites belong only in QA/demo fixtures, not product state; tests should create their own known state instead of relying on production defaults. `src/state.js`'s `fresh()` no longer seeds `selectedFavorites`; every QA script that drives the real app (`bookmark-flow.js`, `a11y-check.js`, `layout-check.js`, the Start Over check in `model-rules.js`) now explicitly selects its own known favorites first. The diagnostic-PR pattern caught 3 follow-on failures (leftover "6" assumptions) on the first pass — fixed and reverified green. Items 4/5 of #59 are still open decisions (see the #59 comment).
- **#60: the favorites-pool addition is in; the followUpPool expansion is intentionally left open, not forced.** Per Paige's instruction: don't force it while local Chrome is flaky — leave it open until Chrome is stable, then expand the pool *and* fix `bookmark-flow.js`'s hardcoded round-size assumptions properly, rather than guessing at a workaround. Nothing further attempted on this here.
- **#28: Paige is good for the ~$0.20 live-model eval once the live producer is ready** — not yet run this session (no immediate trigger; flagging so the next session doesn't need to re-ask). **#15 stays parked** until Paige has time to watch/read/play the candidates — also unchanged.
- **Vercel deployed `357d9e0` successfully** — confirmed both right after merging (status: "deploying") and again after ("Deployment has completed"). The rate limit that blocked earlier deploys this session has cleared.
...
1. Run `npm run test:full` locally when Chrome isn't fighting other sessions on this machine — that's the real next step for #58 (320-390px widths, all four looks) and for #60's followUpPool expansion (needs a `bookmark-flow.js` rewrite to stop hardcoding exact round sizes, done *alongside* the expansion, not before it — both explicitly deferred by Paige until Chrome is stable).
2. Get Paige's read on this session's issue comments and close whichever ones she's happy with — a merge doesn't do that automatically. Full list touched this session: #33, #30, #37, #36, #39, #19, #18, #29, #8, #48, #52, #53, #54, #25, #57, #58, #59, #60.
3. **Merge is being held deliberately** (Paige's call) until #59/#60 are in better shape — item 1 of #59 is now actually done (see above), so the main remaining open piece is #60's followUpPool expansion (item 1 above) plus #59's items 4/5. Ask before merging even once those look done; don't assume "looks done" is the same as "go."
4. Once Paige is ready: get `live-producer-preview`'s five unpromoted commits merged to `main` — same pattern as before, needs her explicit go-ahead each time.
5. **#28** — Paige is already good for the ~$0.20 live-model eval (10 calls) once the live producer is ready; no need to re-ask, just do it and report the result when it's time.
6. **#15** (Experiment 004) — stays parked; waiting on Paige to actually watch/read/play the 5 locked candidates. Nothing to build.
7. **#48** (hard-boundary exclusions) — deliberately left unbuilt; pick up only if Paige wants it.
8. Parked per this session's decisions, revisit only if Paige changes her mind: #6, #7, #9, #10, #11, #17.
