...
- `model-rules.js` — the evidence, Library and Search rules checked directly against `src/model/*` (133 checks). Note: ranking parity against the *original* two-round code was verified once in a scratch Node harness (30,000 random reaction sets) and is not committed.
