...
**State**
- `main` = `d3047e9` (PR #66). **`live-producer-preview` is four commits ahead** (`477a26d` #59 items 4/5, `1dfb5c4` the #28 JSON-parsing fix, `bad8353` #67 overlap fix + #29/#68 Bookmarks visibility, `c403c3e` the Bookmarks-naming decision copy), all verified via clean CI, **not yet promoted** — merge when Paige is ready.
- **Bookmarks-vs-Up-Next decided: keeping "Bookmarks."** Up Next implied ordered intent that overclaims for a no-plan save. Copy warmed up instead (lede + nav tooltip). This was the one open product question from the whole #59/#60/#28/#67/#68 batch — nothing else in it is waiting on a decision from Paige any more.
- **#59 is fully addressed except new-user comprehension testing itself**, which needs a real user, not more code. All 5 concrete items (0 favorites by default, single first-run CTA, de-emphasized nav pre-onboarding, skip the Look picker, copy) are done.
- **A new tracking issue, #68, is the portfolio finish-line checklist** — 6 blocker categories referencing #59/#29/#60/#28/#34+#58+#67/final-QA. Full status posted as a comment on #68 itself; short version: #59 and #28 are essentially done, #29 has one open naming decision (Bookmarks vs. Up Next), #60 and #58 are blocked on local Chrome, final end-to-end QA hasn't started (needs Chrome or a manual pass). **#69** (Music then Art as new domains) is explicitly post-portfolio and not touched — it says so itself.
- **#60: only the favorites-pool addition shipped**, same as before — followUpPool expansion still deliberately held for when Chrome is stable.
- **#28: ran, found a real bug, fixed it.** 4/10 calls succeeded and looked genuinely good (grounded, specific, non-repetitive, correctly cautious — sample reasoning on the #28 comment). The other 6 failed for one traceable reason: the model prefixed its JSON with prose ("Looking at..."), which `parseModelJson` couldn't handle. Fixed both ends (tightened prompt + parser now extracts the first `{...}` object) and added 2 regression tests reproducing the exact failure text pulled from Vercel's logs. **Not yet re-verified against a real model call** post-fix — that costs money and wasn't needed to prove the fix works, but worth a small spot-check once this promotes.
- **#67 (bookmarked-badge overlap) fixed** at the layout-rule level (missing `min-width:0`/`overflow-wrap` on the flex title row let a long/heavy-font title push the reaction stamp outside the card). Not visually screenshot-verified — Chrome's still stuck.
- **#29/#68: Bookmarks tab is now visible even at 0 saved items** (was hidden entirely before). The empty-state copy already existed and was already good. Naming (Bookmarks vs. Up Next) is still an open question for Paige.
- **#15 stays parked** until Paige has time to watch/read/play the candidates.
...
8. **After the merge, two more new issues arrived from a review Paige ran herself: #57 (Look control affordance) and #58 (QA the Look selector), plus #25's acceptance criteria were updated** (same realistic content across all four look previews, plus a "Why this one?" element). All three addressed in `63704df` on `live-producer-preview`, not yet promoted:
   - **#25** — the look preview now renders a real catalog item (`recommendations[0]`) instead of generic placeholder copy, plus a "Why this one?" pill. The four-previews-share-content requirement was already true structurally (one `preview()` function for all four).
   - **#57** — the header Look button now uses the same hover-lift/press-down convention as every other button in the system, plus a small caret, so it reads as "opens something" distinct from the non-interactive PROTOTYPE/AI-READY badge next to it. Keyboard focus and touch target were already correct (confirmed, not changed).
   - **#58 got an honest partial QA report, not a redesign.** Required CI's a11y-check.js has a dedicated "look picker" scope — 0 issues at 1440px/editorial, verified via the diagnostic-PR pattern. But most of #58's actual scope (320-390px widths, all four looks, arrow-key/screen-reader behavior) needs `npm run test:full`, which isn't part of required CI and couldn't be run this session because local Chrome is stuck. Said so plainly on the issue rather than guessing at fixes for unobserved failures.
   - Verified via one more diagnostic PR (#61), closed without merging. All four no-browser suites still pass; no test asserts on the look preview's literal content.
9. **#59 (first-run onboarding) and #60 (broaden the seed catalog) arrived next**, also from Paige's own review, not the earlier product-decision pass:
   - **#59** — new `state.onboarded` flag (true once a visitor has reached Recommendations at least once, set from `navigate()`, `popstate`, and the initial-load path). Before that, Favorites shows one continuation CTA ("Show me my first picks") instead of the two-way Peek-at-my-taste/Show-me-what-I-might-like fork, plus the issue's suggested first-run copy. A returning visitor via "Change favorites" still sees both CTAs. **Explicitly not done:** starting Favorites at 0 selected (item 1 — the 6 `selected: true` catalog items are load-bearing across `model-rules.js`, `bookmark-flow.js`, and `fixtures.mjs` as the reset/restore baseline; needs a real QA-fixture-bypass mechanism, not a guess), deferring the Look picker past first visit (item 5 — the issue frames it as "test," not "do," and a test hardcodes the picker showing first), de-emphasizing nav further (item 4 — needs a design pass, no working local browser this session).
   - **#60** — 4 new unselected favorites-pool items landed clean (`de4a9b0`). The 8 new followUpPool items **did not** — the diagnostic-PR pattern caught `bookmark-flow.js` breaking hard: it hardcodes the followUpPool at exactly 7 items through its entire back half (round-by-round composition, then Bookmarks/My Tastemake/Blind Spots/Taste Map/Search/Library all keyed to specific ids in specific rounds because of that exact size). Reverted the followUpPool part (`f22573e`) rather than land something broken; the fix needs either a rewrite of that part of the flow test to stop hardcoding exact round sizes, or another pass once local Chrome works normally enough to iterate against the real test instead of guessing blind from diagnostic-PR failures.
   - Verified via three diagnostic PRs (#62 caught the regression, #63 confirmed the revert is clean). This is the second real regression the diagnostic-PR pattern has caught this session (see #3 above for the first) — it keeps earning its keep.
...
1. Merge `live-producer-preview`'s three unpromoted commits to `main` when Paige is ready (`477a26d`, `1dfb5c4`, `bad8353`).
2. A small paid spot-check of the #28 fix once promoted: one or two live calls to confirm the tightened prompt + more robust parser actually recover from a live model in production, not just in the regression tests. Cheap, not urgent.
3. Get Paige's read on this session's issue comments and close whichever ones she's happy with — a merge doesn't do that automatically. Full list touched this session: #33, #30, #37, #36, #39, #19, #18, #29, #8, #48, #52, #53, #54, #25, #57, #58, #59, #60, #28, #67, #68.
4. **The Bookmarks-vs-Up-Next naming decision (#29/#68)** is the one real open product question from this batch — small, worth a quick call whenever convenient.
5. Run `npm run test:full` locally when Chrome isn't fighting other sessions on this machine — unblocks #58 (320-390px widths, all four looks), #60's followUpPool expansion (needs a `bookmark-flow.js` rewrite alongside it, not before it), and #67's visual verification. This is now the single biggest lever left for #68's portfolio checklist.
6. Once the above are clean, #68's item 6 (final end-to-end portfolio QA pass) is the last checklist item before Paige can freeze for the case study.
7. **#15** (Experiment 004) — stays parked; waiting on Paige to actually watch/read/play the 5 locked candidates. Nothing to build.
8. **#48** (hard-boundary exclusions) and **#69** (Music/Art domains) — both deliberately left unbuilt; #69 explicitly says it's post-portfolio.
9. Parked per earlier decisions, revisit only if Paige changes her mind: #6, #7, #9, #10, #11, #17.
